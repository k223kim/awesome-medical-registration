...
def _ssim_3D(img1, img2, window, window_size, channel, size_average=True):
    ...
    torch.nn.functional.relu(sigma1_sq, inplace=True)
    sigma2_sq = F.conv3d(img2 * img2, window, padding=window_size // 2, groups=channel) - mu2_sq
    torch.nn.functional.relu(sigma2_sq, inplace=True)
...
class dice(torch.nn.Module):
    ...
    def forward(self, pred_mask, true_mask):
        ...
        true_mask = np.asarray(true_mask.cpu()).astype(np.bool_)
        pred_mask = np.asarray(pred_mask.cpu()).astype(np.bool_)
        
        # If any of the segmentation is empty, the dice will be 1. (Developer decision)
...
class crossCorrelation3D(torch.nn.Module):
    def __init__(self, in_ch, kernel=(9, 9, 9), voxel_weights=None, device_num=0):
        super(crossCorrelation3D, self).__init__()
        self.in_ch = in_ch
        self.kernel = kernel
        self.voxel_weight = voxel_weights
        self.filt = (torch.ones([1, in_ch, self.kernel[0], self.kernel[1], self.kernel[2]])).cuda(device_num)
